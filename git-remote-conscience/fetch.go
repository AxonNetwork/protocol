...
func recurseCommit(sha1 string) error {
	obj, err := fetchObj(sha1)
	if err != nil {
		return errors.Wrapf(err, "fetchObj(%s) commit object failed", sha1)
	}
...
func fetchTree(sha1 string) error {
	obj, err := fetchObj(sha1)
	if err != nil {
		return errors.Wrapf(err, "fetchObj(%s) commit tree failed", sha1)
	}
	entries, ok := obj.Tree()
	if !ok {
	...
	for _, t := range entries {
		obj, err := fetchObj(t.SHA1Sum.String())
		if err != nil {
			return errors.Wrapf(err, "fetchObj(%s) commit tree failed", sha1)
		}
...
func fetchObj(sha1 string) (*git.Object, error) {
	p := filepath.Join(conscienceRepoPath, "objects", sha1[:2], sha1[2:])
